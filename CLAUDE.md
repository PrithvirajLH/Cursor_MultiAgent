...
**510 unit tests (50 suites), 680 integration + 1 skipped, 175 web unit tests (28 files)**, both typechecks clean.

```bash
cd apps/api && npx tsc --noEmit
cd apps/web && npx tsc --noEmit
cd apps/api && npx jest                       # 510, 50 suites
cd apps/web && npx vitest run                 # 175, 28 files

export PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="Yes, reset the local test database"
cd apps/api && npm run test:integration       # 680 + 1 skipped, 66 of 67 suites, ~6-10 min
```

...
**443 unit tests (44 suites), 475 integration + 1 skipped, 100 web unit tests (21 files)**, both typechecks clean.

```bash
cd apps/api && npx tsc --noEmit
cd apps/web && npx tsc --noEmit
cd apps/api && npx jest                       # 443, 44 suites
cd apps/web && npx vitest run                 # 100, 21 files

export PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="Yes, reset the local test database"
cd apps/api && npm run test:integration       # 475 + 1 skipped, takes ~6-10 min
```

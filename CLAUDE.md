...
**470 unit tests (47 suites), 576 integration + 1 skipped, 143 web unit tests (25 files)**, both typechecks clean.

```bash
cd apps/api && npx tsc --noEmit
cd apps/web && npx tsc --noEmit
cd apps/api && npx jest                       # 470, 47 suites
cd apps/web && npx vitest run                 # 143, 25 files

export PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="Yes, reset the local test database"
cd apps/api && npm run test:integration       # 576 + 1 skipped, takes ~6-10 min
```

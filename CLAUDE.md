...
**480 unit tests (48 suites), 617 integration + 1 skipped, 158 web unit tests (26 files)**, both typechecks clean.

```bash
cd apps/api && npx tsc --noEmit
cd apps/web && npx tsc --noEmit
cd apps/api && npx jest                       # 480, 48 suites
cd apps/web && npx vitest run                 # 158, 26 files

export PRISMA_USER_CONSENT_FOR_DANGEROUS_AI_ACTION="Yes, reset the local test database"
cd apps/api && npm run test:integration       # 617 + 1 skipped, takes ~6-10 min
```

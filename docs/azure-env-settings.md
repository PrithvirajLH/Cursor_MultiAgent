...
## Optional – Realtime (Azure Web PubSub)

Use this when you want live ticket/notification updates over WebSocket.

| Name | Example value | Notes |
|------|----------------|-------|
| **AZURE_WEB_PUBSUB_CONNECTION_STRING** | `Endpoint=https://<service>.webpubsub.azure.com;AccessKey=<key>;Version=1.0;` | Required to enable realtime. Leave empty to keep polling-only behavior. |
| **AZURE_WEB_PUBSUB_HUB** | `ticketing` | Hub name used by the API and web client negotiate endpoint. |
| **AZURE_WEB_PUBSUB_TOKEN_LIFETIME_MINUTES** | `60` | Client access token lifetime generated by `/api/realtime/negotiate`. |

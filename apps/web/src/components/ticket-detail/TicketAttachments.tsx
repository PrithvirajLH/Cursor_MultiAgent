...
export function TicketAttachments({
    ...
    return (
      ...
      >
        <div className="max-w-md rounded-2xl border border-dashed border-slate-300 bg-white px-8 py-10 text-center shadow-sm">
          ...
          <h2 className="text-sm font-semibold text-slate-900">No attachments yet</h2>
  ...
  const expandedAttachment =
    ticket.attachments.find((attachment) => attachment.id === expandedAttachmentId) ?? null;
  ...
  async function handleTogglePreview(attachmentId: string) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Unable to preview attachment.';
          ...
          <div className="relative mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950 text-slate-50 shadow-2xl transition-all">
            ...
            <div className="relative flex flex-col gap-4 p-5 sm:p-6">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      onDownloadAttachment(expandedAttachment.id, expandedAttachment.fileName)
                    }
              ...
              <div className="relative mt-1 flex min-h-[220px] max-h-[420px] items-center justify-center overflow-auto rounded-xl bg-slate-900/60">
                {previewLoading ? (
                ...
                ) : expandedAttachment.contentType.startsWith('image/') && previewUrl ? (
                  <img
                    src={previewUrl}
                  ...
                  <p className="text-xs text-slate-400">
                    Inline preview is only available for image attachments. Use Download to open this
                    file.
                  </p>
                )}
              </div>

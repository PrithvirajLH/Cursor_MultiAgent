...
function parseFilters(
  ...
  const scope =
    (searchParams.get("scope") as TicketScope) || presetScope || "all";
  const statusGroup =
...
function filtersToSearchParams(
  ...
  if (filters.requesterIds && filters.requesterIds.length > 0)
  ...
  if (filters.pageSize != null && filters.pageSize !== DEFAULT_PAGE_SIZE)

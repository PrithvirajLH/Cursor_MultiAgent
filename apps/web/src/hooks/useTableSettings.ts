...
function mergeWithDefaults(
  stored: Partial<TableSettings> | null,
): TableSettings {
  const viewMode: ViewMode =
    stored?.viewMode === "table" || stored?.viewMode === "grid"
      ? stored.viewMode
      : "grid";
  ...
  const columnVisibility: Record<TableColumnId, boolean> = {} as Record<
    TableColumnId,
    boolean
  >;
  for (const id of TABLE_COLUMN_IDS) {
    if (
      ...
      typeof stored.columnVisibility[id] === "boolean"
    ) {
...
export function useTableSettings() {
  ...
  const setColumnVisible = useCallback(
    (columnId: TableColumnId, visible: boolean) => {
      setSettings((prev) => {
        const next = {
          ...prev,

...
function resolveTeamId(value: string, teamsList: TeamRef[]): string | null {
  ...
  return (
    teamsList.find((team) => team.name.toLowerCase() === lowered)?.id ?? null
  );
}
...
function resolveTeamAdminScopeTeamName(teamsList: TeamRef[]): string {
  return teamsList.length === 1
    ? teamsList[0].name
    : "Primary Team (unavailable)";
}

function hasConditionGroups(
          ...
          <div>
            ...
            {conditionEditingLocked && (
              <div className="mb-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                This rule uses nested AND/OR groups. Condition editing is locked
                to prevent losing rule logic.
              </div>
            )}
          ...
          <div>
            ...
            <div className="space-y-2">
              {form.actions.map((action, index) => (
                <div
                  key={`action-${index}`}
                  className="flex items-center space-x-2 rounded-lg border border-green-100 bg-green-50 p-2.5"
                >
                  <svg
...
export function AutomationRulesPage({
  ...
  const [assignableUsersHint, setAssignableUsersHint] = useState<string | null>(
    null,
  );
  ...
  async function loadRuleStats(
    ...
    existingMeta: Record<string, RuleUiMeta>,
  ) {
        ...
        try {
          ...
          return [
            rule.id,
            {
              ...
              lastRun: formatRelativeTime(
                execution.data[0]?.executedAt ?? null,
              ),
            },
          ] as const;
        } catch {
  ...
  const totalRuns = useMemo(
    () =>
      sortedRules.reduce(
        (sum, rule) => sum + (uiMetaById[rule.id]?.runCount ?? 0),
        0,
      ),
  ...
  function openEditModal(rule: AutomationRule) {
    ...
    setForm({
      ...
      teamId:
        role === "TEAM_ADMIN"
          ? (rule.teamId ?? teamAdminScopeTeamId)
          : (rule.teamId ?? ""),
  ...
  async function handleSaveRule() {
    ...
    const scopedTeamId =
      role === "TEAM_ADMIN"
        ? teamAdminScopeTeamId || form.teamId || ""
        : form.teamId.trim();
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (
        ...
        const message =
          assignableUsersHint ??
          `Action ${n}: selected user is not in the scoped team.`;
        setError(message);
        toast.error(message);
        return;
      }
    ...
    if (role === "TEAM_ADMIN" && !scopedTeamId) {
      setError(
        "Team admin requires a primary team to manage automation rules.",
      );
      toast.error(
        "Team admin requires a primary team to manage automation rules.",
      );
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex flex-1 items-start space-x-4">
                      <div
                        className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl ${triggerBg(rule.trigger)}`}
                      >
                        <svg
                          className="h-5 w-5"
                          fill="none"

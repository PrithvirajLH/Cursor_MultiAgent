...
import { ArrowLeft, AlignLeft, Calendar, CheckSquare, ChevronDown, Hash, Plus, Type, Users } from 'lucide-react';
...
export function NewCustomFieldPage({
  role,
}: {
  role?: Role;
}) {
  ...
  async function handleSubmit() {
    ...
    if (isTeamAdmin && !resolvedTeamAdminTeamId) {
      const message = 'Team admin requires a primary team to manage custom fields.';
      setError(message);
      toast.error(message);
      return;
    }
    ...
    if ((form.type === 'dropdown' || form.type === 'multiselect') && trimmedOptions.length === 0) {
      const message = 'Dropdown and multi-select fields require at least one option.';
      setError(message);
      toast.error(message);
      return;
    }
      ...
      <div className="mx-auto w-full max-w-[900px] px-6 py-8">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-6 px-6 py-6">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <p className="text-sm font-semibold text-slate-800">Field type</p>
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((prev) => ({
                          ...
                          type: type.value,
                          options:
                            type.value === 'dropdown' || type.value === 'multiselect'
                              ? prev.options
                              : [],
                        }))
                      }
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Limit this field to tickets in a specific category, or keep it global.
                </p>
              </div>
                  ...
                  <input
                    value={
                      resolvedTeamAdminTeamId
                        ? teamsList.find((team) => team.id === resolvedTeamAdminTeamId)?.name ??
                          resolvedTeamAdminTeamId
          ...
          <div className="flex items-center justify-between rounded-b-2xl border-t border-slate-200 bg-slate-50 px-6 py-4">
            <span className="text-xs text-slate-400">
              * Required fields. Changes take effect immediately for new tickets.
            </span>

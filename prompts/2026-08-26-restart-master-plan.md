...
**⚠️ This probably DOES explain the 2026-09-02 incident — reversing what I wrote here.** I argued it probably did not, on the grounds that `isPeerAgent` needs role `AGENT` and that production had `phulgur@` as TEAM_ADMIN. **That premise was mine and it was false.** The decisive check ran 2026-09-03: production has **exactly one AGENT, `phulgur@csnhc.com`, active** — the owner's own account — and **75** unassigned tickets, not 53. So **1.38 was live**, and on any ticket that account was not the assignee of, a reply typed as `Public` was stored `INTERNAL`, emailed to nobody, and confirmed as "Reply sent". If `PA_20260902_046` was unassigned at 19:35, this is the cause of that night and **1.37's display faults were the reason it could not be seen**, not the reason it happened. `apps/api/silent-internal-check.mjs` (read-only) lists every affected message so the owner can tell whether a requester is still waiting. **The irony is the point:** I told the implementer not to reframe 1.37 on an unverified premise, and the unverified premise was mine.

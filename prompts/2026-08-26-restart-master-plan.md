...
| 1.47 Redacted text survives in the outbox | **Queued** — found 2026-09-08 by the planner while verifying 1.11 | — | **Card 1.11's code is right; its claim of completeness is not, and this is the half that matters for PHI.** The implementer's argument for overwriting rather than preserving the body is correct — but it applies just as much to a table neither of us looked at. `NotificationOutbox.body` is a plain `String` holding the **rendered email**, and `notifications.service.ts:998` renders `message.body` straight into it. So for any message that was **already emailed** — exactly the case the redaction caveat warns about — the original text is still in our own database. **The retention job that would delete those rows is off** (confirmed against production 2026-09-08: no retention app settings), so they never age out. The user-facing caveat is honest about the email already having been sent; nobody has been told the copy is also still here. Decide the intent: purge the outbox row's body on redaction, or state plainly that redaction clears the conversation and not the send record. **S.** |

...
| 2026-09-09 | **`INTAKE_API_SECRET` will NOT be rotated for now** — told that the secret had been written to the production container log 252 times in a three-day sample and is the only thing guarding an endpoint excluded from Easy Auth, the owner answered *"i dont want to rotate right now"*. | owner | **Do not rotate it, do not run `rotate-intake-secret.sh`, and do not re-raise it unprompted.** Recorded because it is a decision made with the facts in hand, not an oversight. **Residual risk, stated once and not repeated:** anyone with Kudu read access can post tickets into production until it changes. If it is rotated later, `intake.service.ts` compares a single secret, so the Power Automate flow must be updated in the same window. See card 1.57. |

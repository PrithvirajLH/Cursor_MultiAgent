...
| 1.45 Reports count deleted tickets | **Queued** — found 2026-09-08 by 1.17's implementer, confirmed by the planner | — | **Pre-existing, and the comment in the code says the opposite.** `reports.service.ts:199-201` claims the raw-SQL reports exclude soft-deleted tickets "from `accessConditionSql`". They do not: **reports never calls it.** Of 23 `$queryRaw` reports, only the **three** added by card 1.17 filter `deletedAt` (`:1614`, `:1669`, `:1733`). The other 20 have counted deleted tickets since they were written. ⚠️ **Do not fix it by pointing reports at `accessConditionSql`** — that fragment also carries role/team visibility, which `scopeReportQuery` already does differently, and layering the two would silently change who sees what. Add the clause per report. Impact tracks how many tickets have ever been deleted, so it is small now and grows quietly. **S.** |

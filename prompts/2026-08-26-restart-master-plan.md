...
| 1.49 The ticket-row checkbox does nothing | **Queued** — found 2026-09-08 by 1.12's implementer, pre-existing | — | Clicking the visible checkbox in a ticket row does not select it: the `<td>` carries the toggle and the `<input>` stops propagation, so you have to click the cell **around** the box. Not caused by card 1.12 and deliberately not touched by it. It matters more now than it did last week, because 1.12 just made bulk selection a thing agents will use. **XS.** |
